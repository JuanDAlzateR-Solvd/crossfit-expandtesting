...
|---------|----------|------------------|
| **Login** | `/login` | Username + password fields, Login button. Creds `practice` / `SuperSecretPassword!`. Success → `/secure` ("You logged into a secure area!"); failure → "Invalid username." / "Invalid password." |
